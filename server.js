...
async function fetchReports(species) {
  ...
  // 1. NorCalFishReports — scrape actual recent report entries
  try {
    const res = await fetch('https://www.norcalfishreports.com/fish_reports/saltwater_reports.php', { headers: headers });
    const html = await res.text();
    // Extract report title + body pairs — NorCal uses <td> cells for report content
    const tdMatches = Array.from(html.matchAll(/<td[^>]*class="[^"]*reporttext[^"]*"[^>]*>([\s\S]{40,800}?)<\/td>/gi));
    if (tdMatches.length) {
      for (let i = 0; i < Math.min(2, tdMatches.length); i++) {
        const snip = excerpt(tdMatches[i][1], 350);
        if (snip && snip.length > 40) {
          results.push({ source: 'NorCal Fish Reports', snippet: snip, url: 'https://www.norcalfishreports.com/fish_reports/saltwater_reports.php' });
        }
      }
    }
    // Fallback: find paragraphs with fishing keywords
    if (!results.length) {
      const keywords = species === 'crab' ? 'dungeness|crab' : species === 'halibut' ? 'halibut|flatfish' : species === 'rockfish' ? 'rockfish|lingcod' : 'salmon|chinook|king';
      const re = new RegExp('<p[^>]*>([^<]{60,500}(?:' + keywords + ')[^<]{0,300})<', 'gi');
      const pMatches = Array.from(html.matchAll(re));
      if (pMatches.length) {
        const snip = excerpt(pMatches[0][1], 350);
        if (snip.length > 40) results.push({ source: 'NorCal Fish Reports', snippet: snip, url: 'https://www.norcalfishreports.com/fish_reports/saltwater_reports.php' });
      }
    }
  } catch(e) { console.error('NorCal:', e.message); }

  // 2. CDFW Ocean Sport Fish bulletin — clean government text
  try {
    const res = await fetch('https://wildlife.ca.gov/Fishing/Ocean/Sport-Fish-Bulletins', { headers: headers });
    const html = await res.text();
    const mainMatch = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i);
    if (mainMatch) {
      const snip = excerpt(mainMatch[1], 400);
      if (snip && snip.length > 50) results.push({ source: 'CDFW Sport Fish Bulletin', snippet: snip, url: 'https://wildlife.ca.gov/Fishing/Ocean/Sport-Fish-Bulletins' });
    }
  } catch(e) { console.error('CDFW bulletin:', e.message); }

  // 3. CDFW Dungeness crab status page
  if (species === 'crab') {
    try {
  ...
  // 4. SF Bay Area fishing reports from Western Outdoor News
  try {
    const wonUrl = 'https://www.wonews.com/category/fishing-reports/northern-california-fishing-reports/';
    const res = await fetch(wonUrl, { headers: headers });
    const html = await res.text();
    // Extract article excerpts
    const artMatches = Array.from(html.matchAll(/<article[^>]*>([\s\S]{100,600}?)<\/article>/gi));
    for (let i = 0; i < Math.min(2, artMatches.length); i++) {
      const snip = excerpt(artMatches[i][1], 300);
      if (snip && snip.length > 50) {
        results.push({ source: 'Western Outdoor News', snippet: snip, url: wonUrl });
        break;
      }
    }
  } catch(e) { console.error('WON:', e.message); }
