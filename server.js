...
app.get('/cfc-historical-status', async function(req, res) {
  ...
  res.json({ hasCredentials, cached, in_memory: !!require('./cfc-scraper').getHistoricalSummary });
});
...
app.post('/cfc-historical-build', async function(req, res) {
  ...
  res.json({ status: 'started', message: 'Historical scrape started. Check /cfc-historical-status in 5-10 minutes.' });
  try {
    ...
    console.error('Historical build error:', e.message);
  }
});
