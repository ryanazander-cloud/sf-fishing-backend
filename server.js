...
app.get('/cfc-debug', async function(req, res) {
  ...
  try {
    ...
    const html = await (await require('node-fetch')(
      'https://forums.coastsidefishingclub.com/',
      { headers: { 'User-Agent': 'Mozilla/5.0', 'Cookie': cookie } }
    )).text();
    // Find all node/forum references
    const nodeMatches = html.match(/node-\d+/g) || [];
    const forumMatches = html.match(/forums\/[a-z0-9-]+\.\d+/g) || [];
    const threadMatches = html.match(/threads\/[a-z0-9-]+\.\d+/g) || [];
    // Find saltwater/bay mentions with surrounding context
    const swIdx = html.toLowerCase().indexOf('saltwater');
    const bayIdx = html.toLowerCase().indexOf('bay fishing');
    const swContext = swIdx >= 0 ? html.slice(Math.max(0,swIdx-200), swIdx+300) : 'not found';
    const bayContext = bayIdx >= 0 ? html.slice(Math.max(0,bayIdx-200), bayIdx+300) : 'not found';
    res.json({
      loggedIn: html.includes('data-logged-in="true"'),
      htmlLength: html.length,
      nodeIds: [...new Set(nodeMatches)].slice(0, 30),
      forumUrls: [...new Set(forumMatches)].slice(0, 30),
      threadUrls: [...new Set(threadMatches)].slice(0, 10),
      saltwaterContext: swContext.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim(),
      bayContext: bayContext.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim()
    });
