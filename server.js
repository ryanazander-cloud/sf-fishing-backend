...
app.get('/forecast-species', async function(req, res) {
  ...
  try {
    ...
    const headers2 = { 'User-Agent': 'SFBayFishingApp/1.0' };

    const [nwsRes, tideRes] = await Promise.all([
      fetch('https://forecast.weather.gov/shmrn.php?mz=PZZ540', { headers: headers2 }),
    ...
    // Parse NWS marine text forecast into day blocks
    const nwsText = nwsHtml.replace(/<script[\s\S]*?<\/script>/gi,'').replace(/<[^>]+>/g,' ').replace(/&nbsp;/g,' ').replace(/\s+/g,' ');
    
    function parseMarineDay(txt) {
      const wm = txt.match(/(N|NE|NNE|ENE|E|ESE|SE|SSE|S|SSW|SW|WSW|W|WNW|NW|NNW)\s+wind\s+(\d+)\s*(?:to\s*(\d+))?\s*(?:kt|knots?)/i);
      const sm = txt.match(/[Ss]eas\s+(\d+(?:\.\d+)?)\s*(?:to\s*(\d+(?:\.\d+)?))?\s*(?:ft|feet)/i);
      return {
        wind_kts: wm ? (wm[3] ? Math.round((parseInt(wm[2])+parseInt(wm[3]))/2) : parseInt(wm[2])) : null,
        wind_dir: wm ? wm[1].toUpperCase() : null,
        swell_ft: sm ? (sm[2] ? Math.round((parseFloat(sm[1])+parseFloat(sm[2]))/2*10)/10 : parseFloat(sm[1])) : null
      };
    }

    // Extract day periods from NWS text
    const dayNames = ['TODAY','TONIGHT','MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'];
    const dayMap = {};
    ...
    for (let di = 0; di < 7; di++) {
      ...
      const dowFull = d.toLocaleDateString('en-US', { weekday:'long', timeZone:'America/Los_Angeles' }).toUpperCase();
      
      // Find this day's forecast in NWS text
      let dayText = '';
      const patterns = di === 0 ? ['TODAY'] : [dowFull, dowFull.slice(0,3)];
      for (const pat of patterns) {
        const re = new RegExp(pat + '[\s\S]{10,400}?(?=' + dayNames.join('|') + '|$)', 'i');
        const m = nwsText.match(re);
        if (m) { dayText = m[0]; break; }
      }
      
      const parsed = parseMarineDay(dayText);
      dayMap[dateStr] = Object.assign({ date: dateStr, dow: dow, summary: dayText.slice(0,150).trim() }, parsed);
    }
