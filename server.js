...
app.get('/forecast-species', async function(req, res) {
  ...
  try {
    ...
    // Use the existing /forecast endpoint data which already works
    const [forecastRes, tideRes] = await Promise.all([
      fetch('http://localhost:' + (process.env.PORT || 3000) + '/forecast'),
    ...
    const forecastDays = forecastJson.days || [];

    // Build dayMap from existing forecast data
    const dayMap = {};
