...
app.get('/cfc-historical-status', async function(req, res) {
  ...
  res.json({ hasCredentials, cached, build_status: global.historicalBuildStatus || null });
});
...
app.get('/cfc-index-debug', async function(req, res) {
  ...
  try {
    ...
    const loginPageRes = await fetch('https://forums.coastsidefishingclub.com/login/', { headers: { 'User-Agent': ua } });
    ...
    const body = new URLSearchParams({
      ...
      remember: '1',
      _xfToken: token,
      _xfRedirect: 'https://forums.coastsidefishingclub.com/'
    });
    const loginRes = await fetch('https://forums.coastsidefishingclub.com/login/login', {
      method: 'POST',
      headers: { 'User-Agent': ua, 'Content-Type': 'application/x-www-form-urlencoded', 'Cookie': initCookies, 'Referer': 'https://forums.coastsidefishingclub.com/login' },
      body: body.toString(),
      redirect: 'manual'
    });
    ...
    const hasError = responseText.includes('error') || responseText.includes('incorrect') || responseText.includes('Invalid');
    const errorMatch = responseText.match(/class="[^"]*error[^"]*"[^>]*>([^<]{5,200})/i);
    ...
    res.json({
      ...
      setCookiePreview: setCookie.slice(0, 100),
      hasError: hasError,
      errorMessage: errorMatch ? errorMatch[1].trim() : null,
      responseLength: responseText.length,
      username: process.env.CFC_USERNAME
    });
