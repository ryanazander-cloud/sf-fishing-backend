...
app.get('/forecast-species', async function(req, res) {
  ...
  try {
    ...
    for (const p of periods) {
      ...
      const wm = txt.match(/(N|NE|NW|S|SE|SW|W|E|NNE|NNW|SSE|SSW|ENE|ESE|WNW|WSW)\s+wind[s]?\s+(\d+)\s*(?:to\s*(\d+))?\s*(?:kt|knots?)/i);
      const sm = txt.match(/[Ss]eas\s+(\d+(?:\.\d+)?)\s*(?:to\s*(\d+(?:\.\d+)?))?\s*(?:ft|feet)/);
      dayMap[dateStr] = {
        date: dateStr,
    ...
    const prompt = 'You are an SF Bay Area fishing advisor. For each day below, recommend the single best species to target and the best specific local spot, given the conditions. SF salmon season ' + (salmonOpen ? 'IS open' : 'is CLOSED until June 27') + '. Month: ' + month + '.\n\nGo/No-Go thresholds: max 5ft swell, max 20kt wind, no south wind for ocean.\nKey spots: channel buoys (salmon, incoming tide), Potato Patch (halibut, slack water), Pt. Bonita reefs (rockfish), Raccoon Strait (bay halibut), Duxbury Reef (salmon/rockfish), Gulf of Farallones (needs <4ft swell), South Bay (bay halibut).\n\nDAYS:\n' + dayText + '\n\nFor each day respond with EXACTLY this format (one line per day, no other text):\nMon: [GO/CAUTION/NOGO] | [Species] | [Specific spot] | [Best window if applicable]\n\nIf No-Go just put the reason after NOGO. Be specific with spot names.';
    ...
    // Parse the response into per-day recommendations
    const lines = text.split('\n').filter(function(l){return l.includes('|') || l.includes(':');});
    const recs = {};
    lines.forEach(function(line) {
      const dowMatch = line.match(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)/i);
      if (!dowMatch) return;
      const parts = line.split('|').map(function(p){return p.trim();});
      const verdict = parts[0] ? parts[0].replace(/^[^:]+:\s*/, '').trim() : '';
      recs[dowMatch[1]] = {
        verdict: verdict.startsWith('GO') ? 'go' : verdict.startsWith('CAUTION') ? 'caution' : 'nogo',
        species: parts[1] || '',
        spot: parts[2] || '',
        window: parts[3] || ''
      };
