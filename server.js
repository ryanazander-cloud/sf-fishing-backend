...
app.get('/cfc-debug', async function(req, res) {
  ...
  try {
    // Fetch main forum index to find forum IDs
    const result = await cfc.debugFetch('https://forums.coastsidefishingclub.com/');
    // Filter to just forum links
    const forumLinks = result.links.filter(function(l) { return l.includes('/forums/'); });
    res.json({ loggedIn: result.loggedIn, forumLinks: forumLinks, htmlLength: result.htmlLength });
