...
async function fetchWeather() {
  try {
    ...
    // Swell: prioritize "combined seas" or plain "Seas X to Y ft" — NOT individual wave components
    // This avoids adding NW swell + SW swell together which gives wrong total
    const combinedMatch = text.match(/(?:combined\s+)?[Ss]eas\s+(\d+(?:\.\d+)?)\s*(?:to\s*(\d+(?:\.\d+)?))?\s*(?:ft|feet)/);
    if (combinedMatch) {
      swellFt = combinedMatch[2]
        ? Math.round((parseFloat(combinedMatch[1]) + parseFloat(combinedMatch[2])) / 2 * 10) / 10
        : parseFloat(combinedMatch[1]);
    }

    // Build summary from TODAY section only, strip wave detail lines
    const todayMatch = text.match(/TODAY\s+(.*?)(?=TONIGHT|MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY|SUNDAY|$)/i);
    if (todayMatch) {
      // Remove wave detail breakdown (individual swell components) from summary
      summary = 'TODAY: ' + todayMatch[1]
        .replace(/Wave\s+Detail[^.]*\./gi, '')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 280);
    } else {
      summary = text.slice(0, 250).trim();
    }
...
async function fetchReports(species) {
  ...
  // Build current week URL for NorCal weekly saltwater report
  function getWeekUrl() {
    const now = new Date();
    ...
    const wk = Math.ceil(((sunday - new Date(yr,0,1)) / 86400000 + 1) / 7);
    return 'https://www.norcalfishreports.com/regions/4/saltwater/weekly.php?week=' + yr + '-' + wk;
  }

  // 1. NorCal weekly saltwater report index — has actual boat reports
  try {
    const weekUrl = getWeekUrl();
    const res = await fetch(weekUrl, { headers: headers });
    const html = await res.text();
    // Extract individual report links — format: /fish_reports/[id]/[slug].php
    const linkMatches = Array.from(html.matchAll(/href="(\/fish_reports\/\d+\/[^"]+\.php)"/gi));
    const reportLinks = [...new Set(linkMatches.map(function(m) { return 'https://www.norcalfishreports.com' + m[1]; }))].slice(0, 6);

    for (const link of reportLinks) {
      try {
        const rRes = await fetch(link, { headers: headers });
        const rHtml = await rRes.text();
        // NorCal report pages have report text in <div class="reportbody"> or similar
        const bodyMatch = rHtml.match(/<div[^>]*class="[^"]*(?:reportbody|report_body|report-body|story)[^"]*"[^>]*>([\s\S]{80,800}?)<\/div>/i)
          || rHtml.match(/<p[^>]*>([\s\S]{80,600}?)<\/p>/i);
        if (bodyMatch) {
          const snip = excerpt(bodyMatch[1], 350);
          if (snip && snip.length > 60) {
            // Extract boat/source name from title
            const titleMatch = rHtml.match(/<title>([^<]{5,80}?)<\/title>/i);
            const source = titleMatch ? titleMatch[1].split(' - ')[0].trim() : 'NorCal Fish Reports';
            results.push({ source: source, snippet: snip, url: link });
            if (results.length >= 3) break;
          }
        }
      } catch(e) { /* skip failed individual reports */ }
    }

    // Fallback: grab summary text directly from weekly page
    if (!results.length) {
      const summaryMatches = Array.from(html.matchAll(/<td[^>]*>([\s\S]{80,500}?)<\/td>/gi));
      for (const m of summaryMatches.slice(0, 5)) {
        const snip = excerpt(m[1], 300);
        if (snip && snip.length > 60 && (snip.toLowerCase().includes('halibut') || snip.toLowerCase().includes('salmon') || snip.toLowerCase().includes('rockfish') || snip.toLowerCase().includes('bass') || snip.toLowerCase().includes('fishing'))) {
          results.push({ source: 'NorCal Fish Reports', snippet: snip, url: weekUrl });
          break;
        }
      }
    }
  } catch(e) { console.error('NorCal weekly:', e.message); }

  // 2. NorCal SF Bay spot page — recent reports for this specific location
  try {
    const bayUrl = 'https://www.norcalfishreports.com/spots/1233/san-francisco-bay.php';
    const res = await fetch(bayUrl, { headers: headers });
    const html = await res.text();
    const linkMatches = Array.from(html.matchAll(/href="(\/fish_reports\/\d+\/[^"]+\.php)"/gi));
    const links = [...new Set(linkMatches.map(function(m) { return 'https://www.norcalfishreports.com' + m[1]; }))].slice(0, 3);
    for (const link of links) {
      try {
        const rRes = await fetch(link, { headers: headers });
        const rHtml = await rRes.text();
        const bodyMatch = rHtml.match(/<div[^>]*class="[^"]*(?:reportbody|report_body|story)[^"]*"[^>]*>([\s\S]{80,800}?)<\/div>/i)
          || rHtml.match(/<p[^>]*>([\s\S]{80,600}?)<\/p>/i);
        if (bodyMatch) {
          const snip = excerpt(bodyMatch[1], 350);
          if (snip && snip.length > 60) {
            const titleMatch = rHtml.match(/<title>([^<]{5,80}?)<\/title>/i);
            const source = titleMatch ? titleMatch[1].split(' - ')[0].trim() : 'NorCal — SF Bay';
            if (!results.find(function(r) { return r.snippet === snip; })) {
              results.push({ source: source, snippet: snip, url: link });
  ...
  // 3. CDFW Dungeness crab status
  if (species === 'crab') {
    try {
      const res = await fetch('https://wildlife.ca.gov/Fishing/Ocean/Crab/Dungeness', { headers: headers });
      const html = await res.text();
      const mainMatch = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i);
      if (mainMatch) results.push({ source: 'CDFW Dungeness Crab', snippet: excerpt(mainMatch[1], 400), url: 'https://wildlife.ca.gov/Fishing/Ocean/Crab/Dungeness' });
