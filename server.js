...
function withTimeout(promise, ms) {
  return Promise.race([promise, new Promise(function(_, reject) { setTimeout(function() { reject(new Error('Timeout after ' + ms + 'ms')); }, ms); })]);
}
...
app.get('/reports', async function(req, res) {
  ...
  try {
    // Fetch NorCal reports and CFC reports in parallel
    const [norcalReports, cfcReports] = await Promise.all([
      fetchReports(species),
      (process.env.CFC_USERNAME && process.env.CFC_PASSWORD) ? cfc.getRecentReports().catch(function() { return []; }) : Promise.resolve([])
    ]);

    // Convert CFC reports to standard format and filter by species relevance
    const speciesKeywords = {
      salmon: ['salmon', 'chinook', 'king', 'coho', 'hole', 'buoy', 'trolling', 'herring', 'anchovy'],
      halibut: ['halibut', 'hali', 'flatfish', 'bay', 'ti', 'treasure island', 'raccoon', 'potato patch'],
      rockfish: ['rockfish', 'lingcod', 'rock', 'bottom fish', 'farallon'],
      tuna: ['tuna', 'bluefin', 'albacore', 'bft', 'yellowfin'],
      crab: ['crab', 'dungeness'],
      seabass: ['seabass', 'sea bass', 'wsb', 'white sea']
    };
    const keywords = speciesKeywords[species] || [];

    const cfcFormatted = cfcReports
      .filter(function(r) {
      ...
      .map(function(r) {
        return {
          source: 'CFC — ' + (r.author || 'Member') + (r.date ? ' (' + r.date + ')' : ''),
    ...
    // If no species match, include top CFC reports anyway
    const cfcToUse = cfcFormatted.length ? cfcFormatted : cfcReports.slice(0, 3).map(function(r) {
      return { source: 'CFC — ' + (r.author||'Member') + (r.date?' ('+r.date+')':''), snippet: r.content.slice(0,350), url: r.url, section: r.section };
    });

    const allReports = cfcToUse.concat(norcalReports).slice(0, 6);

    // Generate AI summary
    let summary = null;
    if (allReports.length && process.env.ANTHROPIC_API_KEY) {
      try {
        const reportText = allReports.map(function(r) { return r.source + ': ' + r.snippet; }).join(' | ');
        const prompt = 'Here are recent SF Bay Area fishing reports: ' + reportText + ' Summarize the key fishing intel in 3-4 sentences. Focus on: what species are biting, where, and any notable catch numbers or techniques mentioned. Be specific and direct. No fluff. Write in plain text only — no markdown, no asterisks, no headers, no bullet points.';
    ...
    res.json({ reports: allReports, summary: summary, fetched_at: new Date().toISOString() });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/cfc-historical-status', async function(req, res) {
  const hasCredentials = !!(process.env.CFC_USERNAME && process.env.CFC_PASSWORD && process.env.ANTHROPIC_API_KEY);
  const fs = require('fs');
  const cachePath = (process.env.RAILWAY_VOLUME_MOUNT_PATH || '/tmp') + '/cfc_historical.json';
  let cached = null;
  try {
    if (fs.existsSync(cachePath)) {
      const data = JSON.parse(fs.readFileSync(cachePath, 'utf8'));
      cached = { fetched: data.fetched, age_hours: Math.round((Date.now() - data.fetched) / 3600000), summary_length: data.summary ? data.summary.length : 0, preview: data.summary ? data.summary.slice(0, 300) : null };
    }
  } catch(e) {}
  // Also check weekly summaries cache
  let weeklyCached = null;
  try {
    const weeklyPath = (process.env.RAILWAY_VOLUME_MOUNT_PATH || '/tmp') + '/cfc_weekly.json';
    if (fs.existsSync(weeklyPath)) {
      const wdata = JSON.parse(fs.readFileSync(weeklyPath, 'utf8'));
      const weekCount = Object.keys(wdata.summaries || {}).length;
      const withSummary = Object.values(wdata.summaries || {}).filter(function(w){return w.summary;}).length;
      weeklyCached = { built: wdata.built, week_count: weekCount, weeks_with_summary: withSummary, report_count: wdata.report_count };
    }
  } catch(e) {}
  res.json({ hasCredentials, cached, build_status: global.historicalBuildStatus || null, weekly: weeklyCached });
});

app.get('/cfc-index-debug', async function(req, res) {
  if (!process.env.CFC_USERNAME) return res.json({ error: 'No credentials' });
  try {
    // Use the actual cfcLogin() function which handles cookies properly
    const cookie = await cfc.cfcLogin();
    const fetch = require('node-fetch');
    // Fetch the forum index with the authenticated cookie
    const html = await (await fetch('https://forums.coastsidefishingclub.com/forums/saltwater-fishing-reports.7/', {
      headers: {
    ...
    const threadLinks = (html.match(/href="\/threads\/[^"]+"/g) || []).slice(0, 20);
    const structItemCount = (html.match(/structItem/g) || []).length;
    const cookiePreview = cookie.slice(0, 80);
    // Find what thread listing looks like
    const idx = html.indexOf('structItem');
    const snippet = idx >= 0 ? html.slice(idx, idx + 600).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ') : 'structItem not found';
    res.json({ loggedIn, threadLinks, structItemCount, cookiePreview, snippet, htmlLength: html.length });
  } catch(e) { res.json({ error: e.message }); }
});

app.get('/forecast-species', async function(req, res) {
  if (!process.env.ANTHROPIC_API_KEY) return res.json({ error: 'API key not configured' });
  try {
    const pad = function(n) { return String(n).padStart(2,'0'); };
    const today = new Date();
    ...
    const headers2 = { 'User-Agent': 'SFBayFishingApp/1.0 (fishing@example.com)' };

    // Fetch 7-day gridpoint forecast + tides + today's marine conditions in parallel
    const [nwsRes, tideRes, weatherData] = await Promise.all([
      fetch('https://api.weather.gov/gridpoints/MTR/84,128/forecast', { headers: headers2 }),
      fetch('https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=predictions&application=sf_fishing&begin_date=' + startDate + '&end_date=' + endDate + '&datum=MLLW&station=9414290&time_zone=lst_ldt&interval=hilo&units=english&format=json'),
      fetchWeather()
    ]);
    const nwsJson = await nwsRes.json();
    const tideData = await tideRes.json();
    const tidePreds = tideData.predictions || [];
    const periods = (nwsJson.properties && nwsJson.properties.periods) || [];
    const todayDateStr = today.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });

    function parseWindKts(period) {
      if (!period.windSpeed) return null;
      const m = period.windSpeed.match(/(\d+)(?:\s+to\s+(\d+))?/);
      if (!m) return null;
      const mph = m[2] ? (parseInt(m[1])+parseInt(m[2]))/2 : parseInt(m[1]);
      return Math.round(mph * 0.868976);
    }
    function parseSwellFt(txt) {
      const m = (txt||'').match(/[Ss]eas\s+(\d+(?:\.\d+)?)\s*(?:to\s*(\d+(?:\.\d+)?))?\s*(?:ft|feet)/);
      if (!m) return null;
      return m[2] ? Math.round((parseFloat(m[1])+parseFloat(m[2]))/2*10)/10 : parseFloat(m[1]);
    }

    // Build dayMap from NWS periods (daytime only)
    const dayMap = {};
    for (const p of periods) {
      if (!p.startTime || !p.isDaytime) continue;
      ...
      // Use marine forecast swell for today, gridpoint wind for all days
      const isToday2 = dateStr === todayDateStr;
      dayMap[dateStr] = {
        date: dateStr, dow: dow,
        wind_kts: parseWindKts(p),
        wind_dir: p.windDirection || null,
        swell_ft: isToday2 ? (weatherData.swell_ft || parseSwellFt(txt)) : parseSwellFt(txt),
        summary: (p.shortForecast || txt).slice(0,100)
      };
    }
    // Fill gaps for any missing days
    for (let di = 0; di < 7; di++) {
      const d = new Date(today);
      d.setDate(today.getDate() + di);
      const dateStr = d.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
      const dow = d.toLocaleDateString('en-US', { weekday:'short', timeZone:'America/Los_Angeles' });
      if (!dayMap[dateStr]) dayMap[dateStr] = { date: dateStr, dow: dow, wind_kts: null, wind_dir: null, swell_ft: null, summary: '' };
    }
    ...
    for (const dateStr of Object.keys(dayMap)) {
      const highs = (tidesByDate[dateStr] || []).filter(function(t){return t.type==='H';});
      dayMap[dateStr].high_tides = highs.map(function(t) {
        const parts = t.t.split(' ')[1].split(':');
        const h = parseInt(parts[0]), m = parseInt(parts[1]);
        return (h>12?h-12:(h===0?12:h)) + ':' + String(m).padStart(2,'0') + (h>=12?'pm':'am');
      });
    }

    const days = Object.values(dayMap).sort(function(a,b){return a.date.localeCompare(b.date);}).slice(0,7);
    if (!days.length) return res.json({ days: [] });

    // Ask Claude to score each day with best species + location
    const now = new Date();
    const month = now.toLocaleString('en-US', { month: 'long' });
    const dayOfYear = Math.floor((now - new Date(now.getFullYear(),0,0)) / 86400000);
    // SF salmon season opens June 27
    const salmonOpenDate = new Date(today.getFullYear(), 5, 27); // June 27
    const salmonOpen = today >= salmonOpenDate;

    const dayText = days.map(function(d) {
      return d.dow + ' ' + d.date + ': wind ' + (d.wind_kts||'?') + 'kt ' + (d.wind_dir||'') + ', swell ' + (d.swell_ft||'?') + 'ft, high tides: ' + (d.high_tides||[]).join(' & ');
    }).join('\n');

    const prompt = 'SF Bay Area fishing advisor. Salmon season ' + (salmonOpen ? 'IS open' : 'CLOSED until June 27') + '. Month: ' + month + '. Thresholds: max 5ft swell, 20kt wind. S/SE/SSE/SSW = No-Go. SW ok if under 7kts or clocking W.\nSpots: channel buoys=salmon, Potato Patch=halibut, Pt Bonita reefs=rockfish, Raccoon Strait=bay halibut, Duxbury=salmon/rockfish, Farallones=needs<4ft, South Bay=bay halibut.\n\nDAYS:\n' + dayText + '\n\nRespond with exactly 7 lines, one per day, format:\nSun|GO|Halibut|Potato Patch|morning slack\nMon|NOGO|swell too high\nTue|CAUTION|Rockfish|Pt Bonita|afternoon\n\nUse the exact 3-letter day abbreviation. No other text.';
    ...
    const text = (d2.content && d2.content[0] && d2.content[0].text) || '';

    // Parse pipe-delimited response: Sun|GO|Halibut|Potato Patch|morning slack
    const lines = text.split('\n').filter(function(l){return l.trim().length > 3;});
    const recs = {};
    lines.forEach(function(line) {
      const parts = line.split('|').map(function(p){return p.trim();});
      if (parts.length < 2) return;
      const dowMatch = parts[0].match(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)/i);
      if (!dowMatch) return;
      const v = (parts[1]||'').toUpperCase();
      recs[dowMatch[1]] = {
        verdict: v.startsWith('GO') ? 'go' : v.startsWith('CAUTION') ? 'caution' : 'nogo',
        species: parts[2] || '',
        spot: parts[3] || '',
        window: parts[4] || ''
      };
    ...
    days.forEach(function(d) {
      d.rec = recs[d.dow] || null;
    });

    res.json({ days: days, generated_at: new Date().toISOString() });
  } catch(e) {
    console.error('Forecast species error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

app.get('/trip-planner', async function(req, res) {
  const date = req.query.date; // YYYY-MM-DD
  if (!date) return res.status(400).json({ error: 'date parameter required (YYYY-MM-DD)' });

  const results = cfc.getWeeklySummary(date);
  if (!results || !results.length) {
    return res.json({ date: date, results: [], message: 'No historical data for this date yet. Run the historical build first.' });
  }
  res.json({ date: date, results: results });
});

app.post('/cfc-historical-build', async function(req, res) {
  if (!process.env.CFC_USERNAME || !process.env.CFC_PASSWORD || !process.env.ANTHROPIC_API_KEY) {
    return res.status(503).json({ error: 'Missing credentials' });
  }
  // Store build status in memory
  global.historicalBuildStatus = { started: new Date().toISOString(), status: 'running', error: null, progress: '' };
  res.json({ status: 'started', message: 'Historical scrape started. Check /cfc-historical-status for progress.' });
  try {
    const summary = await cfc.getHistoricalSummary(process.env.ANTHROPIC_API_KEY, true);
    global.historicalBuildStatus = { status: 'complete', completed: new Date().toISOString(), summary_length: summary.length };
    console.log('Historical build complete:', summary.length, 'chars');
  } catch(e) {
    global.historicalBuildStatus = { status: 'error', error: e.message, stack: e.stack };
    console.error('Historical build error:', e.message, e.stack);
  }
});
...
app.post('/briefing', async function(req, res) {
  ...
  try {
    const results = await Promise.all([
      withTimeout(Promise.all([fetchTides(), fetchWeather(), fetchCurrents()]).then(function(r) { return Object.assign({}, r[0], r[1], r[2]); }), 10000),
      withTimeout(fetchReports(species), 8000).catch(function() { return []; })
    ]);
  ...
  // Pull CFC data with timeout — never block briefing waiting for CFC
  let cfcStr = '';
  let historicalStr = '';
  if (process.env.CFC_USERNAME && process.env.CFC_PASSWORD) {
    const withTimeout = function(p, ms) {
      return Promise.race([p, new Promise(function(_, rej) { setTimeout(function() { rej(new Error('CFC timeout')); }, ms); })]);
    };
    try {
      const cfcReports = await withTimeout(cfc.getRecentReports(), 6000);
      if (cfcReports && cfcReports.length) {
        cfcStr = cfcReports.slice(0, 5).map(function(r) {
          return 'CFC - ' + r.author + ' (' + r.date + '): ' + r.content.slice(0, 300);
        }).join('  ');
      }
    } catch(e) { console.error('CFC reports skipped:', e.message); }
    // Only use already-cached historical — never trigger a new build during briefing
    try {
      if (cfc.cfcCache && cfc.cfcCache.historicalSummary) {
        historicalStr = cfc.cfcCache.historicalSummary.slice(0, 2000);
      }
    } catch(e) {}
  }
...
app.get('/forecast-species', async function(req, res) {
  ...
  try {
    ...
    // Fetch marine forecast and tides in parallel
    const [nwsRes, tideRes] = await Promise.all([
      fetch('https://api.weather.gov/zones/forecast/PZZ540/forecast', { headers: headers2 }),
      fetch('https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=predictions&application=sf_fishing&begin_date=' + startDate + '&end_date=' + endDate + '&datum=MLLW&station=9414290&time_zone=lst_ldt&interval=hilo&units=english&format=json')
    ]);
    const nwsJson = await nwsRes.json();
    const tideData = await tideRes.json();
...
app.listen(PORT, '0.0.0.0', function() {
  ...
  // Auto-build historical data on startup — wait 15 seconds for server to stabilize
  if (process.env.CFC_USERNAME && process.env.CFC_PASSWORD && process.env.ANTHROPIC_API_KEY) {
    setTimeout(function() {
      console.log('Auto-starting CFC historical build...');
      global.historicalBuildStatus = { started: new Date().toISOString(), status: 'running', error: null, progress: 'Starting...' };
      cfc.getHistoricalSummary(process.env.ANTHROPIC_API_KEY, true)
        .then(function(summary) {
          global.historicalBuildStatus = { status: 'complete', completed: new Date().toISOString(), summary_length: summary.length };
          console.log('Auto historical build complete:', summary.length, 'chars');
        })
        .catch(function(e) {
          global.historicalBuildStatus = { status: 'error', error: e.message };
          console.error('Auto historical build error:', e.message);
          // Retry once after 30 seconds if login failed
          if (e.message.includes('login failed')) {
            setTimeout(function() {
              console.log('Retrying historical build after login failure...');
              global.historicalBuildStatus.progress = 'Retrying login...';
              cfc.getHistoricalSummary(process.env.ANTHROPIC_API_KEY, true)
                .then(function(s) {
                  global.historicalBuildStatus = { status: 'complete', completed: new Date().toISOString(), summary_length: s.length };
                })
                .catch(function(e2) {
                  global.historicalBuildStatus = { status: 'error', error: e2.message };
...
function getLunarData(date) {
  ...
  const elapsed = (d - knownNewMoon) / (1000 * 60 * 60 * 24);
  const cyclePos = ((elapsed % synodicMonth) + synodicMonth) % synodicMonth;
  const illumination = Math.round((1 - Math.cos(2 * Math.PI * cyclePos / synodicMonth)) / 2 * 100);
  
  // Phase name and icon
  let phase, icon, daysFromNew;
  daysFromNew = cyclePos;
  const daysFromFull = Math.abs(cyclePos - synodicMonth / 2);
  const daysToNew = Math.min(cyclePos, synodicMonth - cyclePos);
  const daysToFull = Math.min(Math.abs(cyclePos - synodicMonth/2), synodicMonth - Math.abs(cyclePos - synodicMonth/2));
  ...
  else if (cyclePos < 14.77) { phase = 'Waxing Gibbous'; icon = '🌔'; }
  else if (cyclePos < 16.61) { phase = 'Full Moon'; icon = '🌕'; }
  else if (cyclePos < 22.15) { phase = 'Waning Gibbous'; icon = '🌖'; }
  else if (cyclePos < 23.99) { phase = 'Last Quarter'; icon = '🌗'; }
  else if (cyclePos < 29.53) { phase = 'Waning Crescent'; icon = '🌘'; }
  else { phase = 'New Moon'; icon = '🌑'; }
  
  // Fishing rating based on moon phase
  let rating, ratingLabel, ratingColor, explanation;
  if (daysToNew <= 2 || daysToFull <= 2) {
    rating = 4; ratingLabel = 'Excellent'; ratingColor = '#2e7d32';
    explanation = daysToNew <= 2 
      ? 'New moon in ' + Math.round(daysToNew) + ' day(s) — spring tides building, strong current, active bite'
      : 'Full moon in ' + Math.round(daysToFull) + ' day(s) — spring tides, strong bait movement, peak activity';
  } else if (daysToNew <= 4 || daysToFull <= 4) {
    rating = 3; ratingLabel = 'Good'; ratingColor = '#558b2f';
    explanation = 'Approaching ' + (daysToNew <= 4 ? 'new' : 'full') + ' moon — tides strengthening, improving conditions';
  } else if (Math.abs(cyclePos - 7.38) < 2 || Math.abs(cyclePos - 22.15) < 2) {
    rating = 2; ratingLabel = 'Fair'; ratingColor = '#f57f17';
    explanation = 'Quarter moon — neap tides, weaker current, slower bite typical';
  } else {
    rating = 2; ratingLabel = 'Fair'; ratingColor = '#f57f17';
    explanation = 'Mid-cycle moon — moderate tidal flow, average fishing conditions';
  }
  
  // Solunar major/minor periods (simplified — moon overhead/underfoot)
  // Moon transit time approximation
  const lunarDay = 24.8427; // hours
  const moonHourAngle = (cyclePos / synodicMonth * 24.8427) % 24.8427;
  const majorPeriod1 = (12 - moonHourAngle % 12.4) % 12.4;
  const majorPeriod2 = (majorPeriod1 + 12.4) % 24;
  const minorPeriod1 = (majorPeriod1 + 6.2) % 24;
  const minorPeriod2 = (majorPeriod1 + 18.6) % 24;
  
  function fmtHour(h) {
    const hh = Math.floor(h) % 24;
    const mm = Math.round((h % 1) * 60);
    const ampm = hh >= 12 ? 'pm' : 'am';
    return (hh > 12 ? hh-12 : (hh===0?12:hh)) + ':' + String(mm).padStart(2,'0') + ampm;
  }
  
  return {
    phase, icon, illumination, cyclePos: Math.round(cyclePos * 10) / 10,
    daysToNew: Math.round(daysToNew * 10) / 10,
    daysToFull: Math.round(daysToFull * 10) / 10,
    rating, ratingLabel, ratingColor, explanation,
    solunar: {
      major: [fmtHour(majorPeriod1), fmtHour(majorPeriod2)],
      minor: [fmtHour(minorPeriod1), fmtHour(minorPeriod2)]
    }
  };
}
...
app.get('/lunar', function(req, res) {
  ...
  const lunar = getLunarData(d);
  // Also get next 7 days
  const week = [];
  for (let i = 0; i < 7; i++) {
    const wd = new Date(d);
    wd.setDate(d.getDate() + i);
    const wl = getLunarData(wd);
    week.push({ date: wd.toISOString().slice(0,10), dow: wd.toLocaleDateString('en-US',{weekday:'short'}), phase: wl.phase, icon: wl.icon, illumination: wl.illumination, rating: wl.rating, ratingLabel: wl.ratingLabel, ratingColor: wl.ratingColor });
  }
  res.json({ today: lunar, week: week });
});
