...
app.post('/briefing', async function(req, res) {
  ...
  // Pull CFC data if credentials available
  let cfcStr = '';
  let historicalStr = '';
  if (process.env.CFC_USERNAME && process.env.CFC_PASSWORD) {
    try {
      const cfcReports = await cfc.getRecentReports();
      if (cfcReports.length) {
        cfcStr = cfcReports.slice(0, 5).map(function(r) {
          return 'CFC - ' + r.author + ' (' + r.date + '): ' + r.content.slice(0, 300);
        }).join('  ');
      }
    } catch(e) { console.error('CFC in briefing:', e.message); }
    try {
      const historical = await cfc.getHistoricalSummary(process.env.ANTHROPIC_API_KEY);
      if (historical) historicalStr = historical.slice(0, 2000);
    } catch(e) { console.error('CFC historical in briefing:', e.message); }
  }
