...
async function fetchWeather() {
  try {
    ...
    // Swell: match "Seas 3 to 5 ft" — the combined sea height line
    // Must appear BEFORE "Wave Detail" section to avoid picking up individual components
    const forecastBody = text.split(/Wave\s+Detail/i)[0]; // cut off at wave detail breakdown
    const combinedMatch = forecastBody.match(/[Ss]eas\s+(\d+(?:\.\d+)?)\s*(?:to\s*(\d+(?:\.\d+)?))?\s*(?:ft|feet)/);
    if (combinedMatch) {
    ...
    // Extract forecast summary — find TODAY block, strip wave detail and GA noise
    const todayMatch = text.match(/TODAY\s+([\s\S]{20,400}?)(?=TONIGHT|MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY|SUNDAY|\.SYNOPSIS|$)/i);
    if (todayMatch) {
      summary = 'TODAY: ' + todayMatch[1]
        .replace(/Wave\s+Detail[\s\S]*?(?=\.[A-Z]|$)/i, '')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 280);
    } else {
      // Fallback: find first meaningful forecast sentence
      const sentenceMatch = text.match(/(?:wind|seas?|swell)[^.]{20,200}\./i);
      summary = sentenceMatch ? sentenceMatch[0].trim() : '';
    }
