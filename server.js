...
app.get('/cfc-index-debug', async function(req, res) {
  ...
  try {
    ...
    // Find all links and key HTML patterns
    const threadLinks = (html.match(/href="\/threads\/[^"]+"/g) || []).slice(0, 20);
    const structItems = (html.match(/class="[^"]*structItem[^"]*"/g) || []).slice(0, 10);
    const hasLoggedIn = html.includes('data-logged-in="true"');
    // Get a snippet around 'structItem'
    const idx = html.indexOf('structItem');
    const snippet = idx >= 0 ? html.slice(idx, idx + 500) : 'structItem not found';
    res.json({ hasLoggedIn, threadLinks, structItems, snippet: snippet.replace(/<[^>]+>/g,' ').replace(/\s+/g,' '), htmlLength: html.length });
