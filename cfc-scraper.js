...
const COOKIE_CACHE_FILE = path.join(CACHE_DIR, 'cfc_cookie.json');

function loadCookieCache() {
  try {
    if (fs.existsSync(COOKIE_CACHE_FILE)) {
      const data = JSON.parse(fs.readFileSync(COOKIE_CACHE_FILE, 'utf8'));
      if (data.cookie && data.expires && new Date(data.expires) > new Date()) {
        cfcCache.cookie = data.cookie;
        cfcCache.cookieExpires = new Date(data.expires);
        console.log('Loaded CFC cookie from file cache');
      }
    }
  } catch(e) {}
}

function saveCookieCache(cookie, expires) {
  try {
    fs.writeFileSync(COOKIE_CACHE_FILE, JSON.stringify({ cookie: cookie, expires: expires.toISOString() }));
  } catch(e) {}
}
...
async function cfcLogin() {
  ...
  // Use raw headers to get all set-cookie values properly
  const rawHeaders = loginRes.headers.raw ? loginRes.headers.raw() : {};
  const setCookieArr = rawHeaders['set-cookie'] || [];
  
  let cookies = '';
  if (setCookieArr.length > 0) {
    // Extract name=value from each header (before first semicolon)
    cookies = setCookieArr.map(function(c) { return c.split(';')[0].trim(); }).filter(Boolean).join('; ');
  } else {
    // Fallback: single header — but split carefully (not on commas in dates)
    const single = loginRes.headers.get('set-cookie') || '';
    if (single) cookies = single.split(';')[0].trim();
  }
  
  // Merge init cookies
  if (initCookies) {
    const initParsed = (rawHeaders['set-cookie'] ? [] : initCookies.split(','))
      .map(function(c) { return c.split(';')[0].trim(); }).filter(Boolean).join('; ');
    if (initParsed) cookies = cookies ? cookies + '; ' + initParsed : initParsed;
  }

  if (!cookies || cookies.length < 10) {
    throw new Error('CFC login failed — check credentials');
  }
  const cookieExpiry = new Date(Date.now() + 4 * 60 * 60 * 1000);
  cfcCache.cookie = cookies;
  cfcCache.cookieExpires = cookieExpiry;
  saveCookieCache(cookies, cookieExpiry);
  return cookies;
}
...
async function getHistoricalSummary(anthropicKey, forceRefresh) {
  ...
  // Always reuse existing cookie if valid — don't force re-login
  const cookie = await cfcLogin();
  const allReports = [];
  ...
  for (const forumConfig of [
    ...
    for (let page = 1; page <= 50; page++) {
      try {
        ...
        // Extract thread links and dates from forum index page
        // XenForo: thread links are /threads/slug.ID/ format
        const seen = new Set(threadMeta.map(function(t){return t.url;}));
        
        // Get all dates on the page (one per thread roughly)
        const pageDates = Array.from(html.matchAll(/datetime="([^"T]+)/gi)).map(function(m){return m[1].slice(0,10);}).filter(function(d){return d.length===10;});
        
        // Get all thread links with their titles
        const linkRe = /href="(\/threads\/([^"\/]+)\.\d+\/)"[^>]*>([^<]{3,120})<\/a>/gi;
        let li = 0;
        let lm;
        while ((lm = linkRe.exec(html)) !== null) {
          ...
          const threadUrl = CFC_BASE + rawUrl.replace(/\/(latest|unread|page-\d+)\/?$/, '/');
          if (seen.has(threadUrl)) continue;
          if (/test.posting|introduce.yourself|herring.spawn|rules.and.reg/i.test(title)) { li++; continue; }
          seen.add(threadUrl);
          threadMeta.push({ url: threadUrl, title: title, date: pageDates[li] || '', section: forumConfig.section, preview: '' });
          li++;
        }
    ...
    // Sample threads with species-aware balancing for good coverage
    const sampled = [];
    const speciesTerms = ['salmon', 'chinook', 'king', 'rockfish', 'lingcod', 'tuna', 'albacore', 'bluefin', 'crab', 'dungeness', 'seabass', 'halibut', 'striper', 'bass'];
    
    if (threadMeta.length <= 100) {
      sampled.push(...threadMeta);
    } else {
      // First: include all threads with species keywords in title (up to 60)
      const speciesThreads = threadMeta.filter(function(t) {
        const lower = t.title.toLowerCase();
        return speciesTerms.some(function(s) { return lower.includes(s); });
      }).slice(0, 60);
      speciesThreads.forEach(function(t) { sampled.push(t); });
      
      // Then: fill remaining slots with evenly-spaced sample for date coverage
      const remaining = threadMeta.filter(function(t) {
        return !sampled.find(function(s) { return s.url === t.url; });
      });
      const step = Math.max(1, Math.floor(remaining.length / (100 - sampled.length)));
      for (let i = 0; i < remaining.length && sampled.length < 100; i += step) {
  ...
  // Targeted salmon pass — hunt specifically for salmon/chinook threads May-August
  // Go through all pages looking for salmon keywords in thread titles
  console.log('Starting targeted salmon pass...');
  if (global.historicalBuildStatus) global.historicalBuildStatus.progress = 'Running targeted salmon search...';
  const salmonKeywords = ['salmon', 'chinook', 'king salmon', 'opener', 'coho', 'silvers'];
  const salmonUrls = new Set(allReports.map(function(r) { return r.url || ''; }));
  
  for (const forumUrl of [CFC_SALTWATER_URL]) {
    for (let page = 1; page <= 200; page++) {
      try {
        const url = forumUrl + (page > 1 ? '?page=' + page : '');
        const html = await fetchCFCPage(url, cookie);
        
        // Find salmon-related threads on this page
        const linkRe = /href="(\/threads\/([^"\/]+)\.\d+\/)"[^>]*>([^<]{3,120})<\/a>/gi;
        let foundSalmon = false;
        let lm;
        const pageDates = Array.from(html.matchAll(/datetime="([^"T]+)/gi)).map(function(m){return m[1].slice(0,10);}).filter(function(d){return d.length===10;});
        let li = 0;
        ...
        while ((lm = linkRe.exec(html)) !== null) {
          ...
          const isSalmon = salmonKeywords.some(function(k) { return lower.includes(k); });
          if (isSalmon) {
            const threadUrl = CFC_BASE + lm[1].replace(/\/(latest|unread|page-\d+)\/?$/, '/');
            if (!salmonUrls.has(threadUrl)) {
              salmonUrls.add(threadUrl);
              foundSalmon = true;
              // Fetch the thread
              try {
                const threadHtml = await fetchCFCPage(threadUrl, cookie);
                const posts = extractPostContent(threadHtml);
                if (posts.length) {
                  allReports.push({
                    title: title,
                    date: posts[0].date || pageDates[li] || '',
                    section: 'Saltwater',
                    content: posts[0].text.slice(0, 400),
                    species_targeted: 'salmon'
                  });
                }
                await new Promise(function(r) { setTimeout(r, 150); });
              } catch(e) { /* skip */ }
        ...
        if (global.historicalBuildStatus) global.historicalBuildStatus.progress = 'Salmon search page ' + page + ' - ' + allReports.length + ' total reports';
        await new Promise(function(r) { setTimeout(r, 150); });
        
        // Stop if page has no thread links at all
        if (!html.includes('/threads/')) break;
        
      } catch(e) {
        console.error('Salmon pass page error:', e.message);
        break;
      }
    }
  }
  
  console.log('After salmon pass:', allReports.length, 'total reports');
  ...
  console.log('Starting weekly summary build with', allReports.length, 'reports...');
  try {
    await buildWeeklySummaries(anthropicKey, allReports);
...
module.exports = { getRecentReports, getHistoricalSummary, buildWeeklySummaries, getWeeklySummary, getWeekLabel, getWeekOfYear, cfcLogin, debugFetch, cfcCache: cfcCache };
