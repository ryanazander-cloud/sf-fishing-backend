...
async function cfcLogin() {
  ...
  if (!setCookie || setCookie.length < 10) {
    // Login may have succeeded but no new cookie — try using init cookies
    if (initCookies && initCookies.length > 10) {
      const fallback = initCookies.split(',').map(function(c) { return c.split(';')[0].trim(); }).join('; ');
      const fallbackExpiry = new Date(Date.now() + 4 * 60 * 60 * 1000);
      cfcCache.cookie = fallback;
      cfcCache.cookieExpires = fallbackExpiry;
      saveCookieCache(fallback, fallbackExpiry);
      return fallback;
    }
