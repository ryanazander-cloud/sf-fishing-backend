...
const CFC_BASE = 'https://www.coastsidefishingclub.com';
const CFC_LOGIN_URL = CFC_BASE + '/login/login';
const CFC_REPORTS_URL = CFC_BASE + '/forums/saltwater-fishing-reports.5/';
...
function extractThreadLinks(html) {
  // XenForo thread links: /threads/title.12345/
  const matches = html.matchAll(/href="(\/threads\/[^"]+\.(\d+)\/?)"/gi);
  const links = [];
  const seen = new Set();
  for (const m of matches) {
    const url = CFC_BASE + m[1].split('?')[0];
...
module.exports = { getRecentReports, getHistoricalSummary, cfcLogin };
